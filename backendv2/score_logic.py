...
def get_score(team, cap) -> str:

    frame = crop_score(team, cap)
    frame = clean_frame(frame)

    custom_config = r'--psm 13 -c tessedit_char_whitelist=0123456789'
    score = image_to_string(frame, config=custom_config)
    score = score.replace('\n', '').replace('\f', '')

    # if not score:
    #     frame = crop_score(team, cap)
    #     custom_config = r'--oem 3 --psm 6 -c tessedit_char_whitelist=0123456789 outputbase digit'
    #     score = loop.run_in_executor(None, lambda: image_to_string(frame, config=custom_config))
    #     score = score.replace('\n', '').replace('\f', '')
    #     if not score:
    #         score = "0"
...
if __name__ == '__main__':
    ...
    frame = crop_score(settings['score_right_position'], xcap)
